...
class OpenAIProvider(BaseProvider):
    ...
    async def discover_models(self) -> list[str]:
        ...
        try:
            ...
            return [model_id for model_id in ids if "gpt" in model_id.lower()]
        except Exception:
            return []
    ...
    async def generate(self, model_name: str, prompt: str,
        ...
        try:
            ...
            return GenerateResult(
                content=f"ERROR: OpenAI returned {code}",
                model=model_name, provider="openai", error=error_type,
            )
        except Exception as e:
            return GenerateResult(
                content=f"ERROR: {str(e)}",
                model=model_name, provider="openai", error="unknown",
            )
