    ...
    async def generate(self, model_name: str, prompt: str,
        ...
        if not self.api_key:
            return GenerateResult(
                content="ERROR: No Perplexity API key configured",
                model=model_name, provider="perplexity", error="no_api_key",
            )
        ...
        try:
            ...
            return GenerateResult(
                content=content, model=model_name, provider="perplexity",
            ...
            return GenerateResult(
                content=f"ERROR: Perplexity returned {code}",
                model=model_name, provider="perplexity", error=error_type,
            )
        except Exception as e:
            return GenerateResult(
                content=f"ERROR: {str(e)}",
                model=model_name, provider="perplexity", error="unknown",
            )
